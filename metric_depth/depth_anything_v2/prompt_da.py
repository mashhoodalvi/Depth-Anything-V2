...
from .util.blocks import FeatureFusionBlock, _make_scratch, FeatureFusionDepthBlock
...
class PromptDA(nn.Module):
    def __init__(
        ...
        self.depth_head = DPTHead(self.pretrained.embed_dim, features=features, use_bn=use_bn, out_channels=out_channels, use_clstoken=use_clstoken)
    
    def forward(self, x, depth_prior):
        patch_h, patch_w = x.shape[-2] // 14, x.shape[-1] // 14
        depth_prior = depth_prior / self.max_depth
        
        features = self.pretrained.get_intermediate_layers(x, self.intermediate_layer_idx[self.encoder], return_class_token=True) 

        
        depth = self.depth_head(features, patch_h, patch_w, depth_prior) * self.max_depth
