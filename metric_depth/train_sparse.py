...
parser.add_argument('--pretrained-from', default='metric_depth/checkpoints/depth_anything_v2_metric_hypersim_vitb.pth', type=str)
parser.add_argument('--save-path', default="./outputs", type=str)#, required=True)
...
def main():
    ...
    relative_path = os.path.abspath(os.path.join(filepath, args.relative_path))

    size = (args.img_size, args.img_size)
    if args.dataset == 'hypersim':
        trainset = Hypersim(os.path.join(filepath,'dataset/splits_sparse/hypersim/train.txt'), 'train', relative_path, size=size)
    ...
    if args.dataset == 'hypersim':
        valset = Hypersim(os.path.join(filepath,'dataset/splits_sparse/hypersim/val.txt'), 'val', relative_path, size=size)
    elif args.dataset == 'vkitti':
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample in enumerate(trainloader):
            ...
            img, depth, valid_mask, prior = sample['image'].to(device), sample['depth'].to(device), sample['valid_mask'].to(device), sample['prior'].to(device)
        ...
        for i, sample in enumerate(valloader):
            ...
            img, depth, valid_mask, prior = sample['image'].to(device).float(), sample['depth'].to(device)[0], sample['valid_mask'].to(device)[0], sample['prior'].to(device)
